...
This action supports s3 mirroring. This means the [`ecs-service`](https://github.com/cloudposse/terraform-aws-components/tree/main/modules/ecs-service) component will upload a task definition template to S3, this action will use and merge that template with the task definition from the repository and then deploy the merged task definition to ECS.
